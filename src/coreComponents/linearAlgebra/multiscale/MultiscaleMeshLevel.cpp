                                   ...
                                   localIndex const numLocalNodes )
{
  ...
  forAll< serialPolicy >( nodeManager.size(), [&]( localIndex const k )
  {
    ...
    if( newGlobalIndex >= 0 )
    {
      localIndex const localIndexTmp = LvArray::integerConversion< localIndex >( newGlobalIndex - rankOffset );
                                   ...
                                   localIndex const numLocalCells )
{
  ...
  elemManager.forElementSubRegions( regions, [&]( localIndex, ElementSubRegionBase & subRegion )
  {
    ...
    {
      localIndex const localIndexTmp = LvArray::integerConversion< localIndex >( cellGlobalIndex[ei] - rankOffset );
                                       ...
                                       ArrayOfSetsView< globalIndex const > const & nodeToSubdomain )
{
  ...
  ArrayOfSets< localIndex > const featureAdjacency( numFeatures, 32 );
    ...
    featureAdjacency = featureAdjacency.toView()]( localIndex const f )
  {
    ...
    {
      meshUtils::forUniqueNeighbors< 128 >( inf, nodeToCell, cellToNode, 1, [&]( localIndex const nbrIdx )
      {
...
void MultiscaleMeshLevel::writeCellDataCoarse( std::vector< string > const & fieldNames ) const
{
  ...
  for( string const & fieldName : fieldNames )
  {
                                    ...
                                    [&]( auto arrayInstance, auto const )
    {
      ...
      auto dstField = m_fineMesh->cellManager().getReference< ArrayType >( fineFieldName ).toView();

      forAll< parallelHostPolicy >( dstField.size(), [=]( localIndex const ic )
      {
...
void MultiscaleMeshLevel::writeNodeDataFine( std::vector< string > const & fieldNames ) const
{
  ...
  for( string const & fieldName : fieldNames )
  {
                                    ...
                                    [&]( auto arrayInstance, auto const )
    {
      ...
      auto const dstField = m_sourceMesh->getNodeManager().getReference< ArrayType >( sourceFieldName ).toView();

      forAll< parallelHostPolicy >( m_nodeManager.size(), [=]( localIndex const ic )
      {
        LvArray::forValuesInSliceWithIndices( srcField[ ic ],
                                              [&]( auto const & sourceVal, auto const ... indices )
        {
...
void MultiscaleMeshLevel::writeNodeDataCoarse( std::vector< string > const & fieldNames ) const
{
  ...
  for( string const & fieldName : fieldNames )
  {
                                    ...
                                    [&]( auto arrayInstance, auto const )
    {
      ...
      auto dstField = m_fineMesh->nodeManager().getReference< ArrayType >( fineFieldName ).toView();
